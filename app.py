...
df_google = pd.read_csv('US_Corona_New.csv')
...
opts = [{'label' : i, 'value' : i} for i in state]
...
fig.update_layout(height=500, width=700, 
                    title = 'Time Series Plot for Mobility in' + str(df0['State'][0]),
                    hovermode = 'closest', 
...
app.layout = html.Div([
                ...
                html.P([
                 ...
                 html.P([
                    ...
                    dcc.Dropdown(id = 'opt_s', options = opts,
...
def update_figure(input2, selected_state):
    df_3 = df_google[(df_google['State'] == selected_state)]
...
#### Google Mobility
    ...
    if (df_3['State'][0] == "The Whole Country"):
        fig.update_layout(height=500, width=700, 
                    title = 'Time Series Plot for Mobility in ' + str(df_4['State'][0]),
                    hovermode = 'closest')
        ...
        fig.update_layout(height=500, width=700, 
                        title = 'Time Series Plot for Mobility in ' + str(df_4['State'][0]) + ', ' + str(df_3['State'][0]),
                        hovermode = 'closest')
